...
if (!RAW_BACKEND_BASE_URL) {
  // Fail loudly during dev/build so a missing env var can never silently point at the wrong backend.
  // Set VITE_BACKEND_BASE_URL in .env.development / .env.production (or .env.local).
  throw new Error(
    "VITE_BACKEND_BASE_URL is not defined. Set it in your .env file (e.g. http://localhost:8081/api for dev)."
  );
}
