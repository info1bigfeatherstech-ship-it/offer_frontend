...
export function validateAddressFormStep2(form) {
  ...
  if (!trim(form.building)) {
    return { ok: false, message: "Building name is required." };
  }
  if (trim(form.building).length > 150) {
    return { ok: false, message: "Building name must be at most 150 characters." };
  }
  if (!trim(form.floor)) {
    return { ok: false, message: "Floor details are required." };
  }
  if (trim(form.floor).length > 80) {
