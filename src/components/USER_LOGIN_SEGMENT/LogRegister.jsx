...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   CHANGED (unified white theme):
   • lr-modal-card background changed from #0d0d0d to #ffffff — all panels
     (login, register, forgot, otp) now sit on a white card.
   • lr-panel-body updated to white background + light gray scrollbar track
     so the scrollable area matches the white card.
   • Tab bar and mobile pill/close bar backgrounds updated to #ffffff with
     dark text/border so they remain readable on white.
   • isForgotView conditional logic removed — no longer needed since everything
     is white. lr-panel-body-white class also removed (unified into lr-panel-body).
   • All other logic, functionality, swipe handling, scroll handling, viewport
     height fix, browser back button handler, and every commented line preserved.
─────────────────────────────────────────────────────────────────────────────*/
    ...
    <>
      ...
      <style>{`
        ...
        /* ── Modal card: flex column so tab bar sticks and content scrolls ──
           CHANGED: background #0d0d0d → #ffffff (unified white theme)
           border updated to light gray, shadow softened for white card */
        .lr-modal-card {
          display: flex;
        ...
        /* ── Scrollable panel body (login / register / forgot / OTP) ──
           CHANGED: background #ffffff, light gray scrollbar track to match white card.
           lr-panel-body-white removed — this single class now serves all panels. */
        .lr-panel-body {
          flex: 1 1 0%;
          min-height: 0;
          overflow-y: auto;
      ...
      >
        ...
        >
          {/* Desktop close button — outside card so it's never clipped
              CHANGED: border-[#0d0d0d] → border-white to match white card edge */}
          <button
            onClick={handleClose}
            className="absolute -top-3 -right-3 z-20 hidden sm:flex items-center justify-center bg-[#f7a221] text-black p-2 rounded-full shadow-xl border-2 border-white cursor-pointer active:scale-95 transition-transform"
            aria-label="Close"
          >
          ...
          <div className="lr-modal-card w-full min-h-0">
            ...
            {/* Mobile drag pill + close — flex-shrink:0, never scrolls.
                CHANGED: unified white bg/pill/close for all views — no more
                conditional isForgotView switching needed. */}
            <div
              className="flex-shrink-0 flex items-center px-4 pt-3 pb-0 sm:hidden"
              style={{ background: "#ffffff" }}
            >
              <div className="flex-1 flex justify-center pl-6">
                <div className="w-8 h-1 rounded-full" style={{ background: "#e0e0e0" }} />
              </div>
              <button
                onClick={handleClose}
                className="p-1.5 rounded-full active:scale-90 transition-transform cursor-pointer touch-manipulation"
            ...
            {currentView === "tabs" && (
              <>
                {/* Tab bar — flex-shrink:0 so it sticks at top
                    CHANGED: bg-[#0d0d0d] → #ffffff, border-white/5 → #e8e8e8,
                    tab text colors adapted for white background */}
                <div
                  className="flex-shrink-0 flex relative mt-1 sm:mt-0"
                ...
                >
                  <button
                    ...
                    style={{ color: activeTab === "login" ? "#f7a221" : "#aaaaaa" }}
                  >
                    LOGIN
                  </button>
                  <button
                    ...
                    style={{ color: activeTab === "register" ? "#f7a221" : "#aaaaaa" }}
                  >
                    REGISTER
                  </button>
                  {/* Animated underline */}
...
// down code is working but upper code have diff theme for all the pages  
// import React, { useState, useRef, useCallback, useEffect } from "react";
// import { X } from "lucide-react";
// import Login from "./Login";
// import Register from "./Register";
