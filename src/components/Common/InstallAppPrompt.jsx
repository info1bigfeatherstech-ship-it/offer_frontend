...
/**
 * Single install popup — at most once per page load (StrictMode-safe).
 */
const InstallAppPrompt = ({
  ...
  const setPromptVisible = useCallback((next, nextMode = 'open') => {
    setVisible(next);
    if (next) setMode(nextMode);
    onVisibilityChangeRef.current?.(Boolean(next));
  }, []);
  ...
  useEffect(() => {
    ...
    const onInstalled = () => {
      ...
      // Attribute only when logged in; pending flag covers login-after-install.
      markPwaInstallPendingAttribution();
    ...
    // One delayed open. Cleanup clears timer (StrictMode-safe).
    // claimOpenPromptShow() ensures only one visible open even if two timers race.
    const openTimer = window.setTimeout(() => {
      if (!canOfferInstall() || openDismissedThisVisitRef.current) {
        onVisibilityChangeRef.current?.(false);
        return;
      }
      if (!claimOpenPromptShow()) return;
          ...
          >
            ...
            >
              ...
              <div className="relative flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain px-4 pb-4 pt-5 sm:px-6 sm:pb-6 sm:pt-8">
                ...
                <p className="mt-3 text-center text-[10px] font-semibold uppercase tracking-[0.16em] text-gray-400 sm:mt-3.5 sm:text-[11px] sm:tracking-[0.18em]">
                  {brandName}
                </p>
                ...
                <p className="mx-auto mt-1.5 max-w-[18rem] text-center text-[12px] leading-relaxed text-gray-500 sm:mt-2 sm:max-w-none sm:text-sm">
                  {isIos
                    ? 'Tap Share → Add to Home Screen for a full-app shopping experience.'
                    : 'Faster Shopping · Home Screen · Deal Alerts'}
                </p>

                <div className="mt-4 flex w-full flex-col gap-1.5 sm:mt-6 sm:gap-2">
                  <button
                    ref={installBtnRef}
                    type="button"
