...
const Footer = () => {
  ...
  // Only active, split into 2 equal columns dynamically
  const activeCategories = allCategories.filter(cat => cat.status === 'active');
  const half = Math.ceil(activeCategories.length / 2);
  const col1 = activeCategories.slice(0, half);
  const col2 = activeCategories.slice(half);
...
//         { label: "Beauty & Personal Care", path: "/category/beauty-and-personal-care" },
//         { label: "Home Improvement", path: "/category/home-improvement" },
//       ]
//     },
//     {
//       title: "Quick links",
